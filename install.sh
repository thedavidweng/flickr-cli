...
# flickr-cli installer
# Usage: curl -fsSL https://raw.githubusercontent.com/thedavidweng/flickr-cli/main/install.sh | sh

REPO="thedavidweng/flickr-cli"
BINARY="flickr"
CASK="thedavidweng/tap/flickr"
...
install_binary() {
  ...
  bin_dir="${FLICKR_INSTALL_DIR:-$HOME/.local/bin}"
  mkdir -p "$bin_dir"
...
# --- Uninstall helper ---
uninstall_brew() {
  step "Uninstalling Homebrew-managed flickr-cli"
...
case "${1:-}" in
  uninstall)
    if has_brew && brew list --cask "flickr" >/dev/null 2>&1; then
      uninstall_brew
    else
      uninstall_binary
    fi
    step "Uninstalled. You may also remove flickr-cli config from ~/.config/flickr-cli/"
    exit 0
    ;;
  --help|-h)
    cat <<EOF
...
Installs flickr-cli. Prefers Homebrew Cask if available, otherwise
downloads the binary to ~/.local/bin.

Environment:
  FLICKR_INSTALL_DIR  Directory for binary (default: ~/.local/bin)

Options:
  uninstall    Remove flickr-cli
...
step "Installing flickr-cli ($platform_label)"

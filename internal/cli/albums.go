...
var albumsCreateCmd = &cobra.Command{
	...
	RunE: withAuth("albums.create", func(ctx *CmdContext) error {
		title, _ := ctx.Cmd.Flags().GetString("title")
		description, _ := ctx.Cmd.Flags().GetString("description")
		primaryPhotoID, _ := ctx.Cmd.Flags().GetString("primary-photo-id")

		if title == "" {
			return ctx.R.Failure(ctx.Meta, output.Errorf(model.ErrValidationFailed, "title is required"))
		}
		if primaryPhotoID == "" {
			return ctx.R.Failure(ctx.Meta, output.Errorf(model.ErrValidationFailed, "primary-photo-id is required"))
		}

		return ctx.runMutation(mutationSpec{
			Command:  "albums.create",
			Method:   "flickr.photosets.create",
			Resource: map[string]any{"title": title, "primary_photo_id": primaryPhotoID},
			PlanMsg:  fmt.Sprintf("Would create album %q with primary photo %s\n", title, primaryPhotoID),
			PlanData: map[string]any{"title": title, "primary_photo_id": primaryPhotoID},
		}, func() (any, error) {
			params := map[string]string{"title": title, "primary_photo_id": primaryPhotoID}
			if description != "" {
			...
			if err := ctx.Client.Call(ctx.Cmd.Context(), "flickr.photosets.create", params, &result); err != nil {
				return nil, ctx.R.Failure(ctx.Meta, output.Errorf(model.ErrFlickrAPI, "%v", err))
			}
			ctx.R.Human("Created album %s\n", result.Photoset.ID)
			return map[string]any{"id": result.Photoset.ID, "title": title}, nil
		})
	}),
}
...
var albumsUpdateCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: withAuth("albums.update", func(ctx *CmdContext) error {
		albumID := ctx.Args[0]
		title, _ := ctx.Cmd.Flags().GetString("title")
		description, _ := ctx.Cmd.Flags().GetString("description")
		primaryPhotoID, _ := ctx.Cmd.Flags().GetString("primary-photo-id")
		...
		return ctx.runMutation(mutationSpec{
			...
			Resource: map[string]any{"album_id": albumID},
			PlanMsg:  fmt.Sprintf("Would update album %s\n", albumID),
			PlanData: map[string]any{"id": albumID},
		}, func() (any, error) {
			params := map[string]string{"photoset_id": albumID}
			if title != "" {
				params["title"] = title
			}
			if description != "" {
			...
			if err := ctx.Client.Call(ctx.Cmd.Context(), "flickr.photosets.editMeta", params, nil); err != nil {
				return nil, ctx.R.Failure(ctx.Meta, output.Errorf(model.ErrFlickrAPI, "%v", err))
			}
			ctx.R.Human("Updated album %s\n", albumID)
			return map[string]any{"id": albumID}, nil
		})
	}),
}
...
var albumsDeleteCmd = &cobra.Command{
	...
	Args:  cobra.ExactArgs(1),
	RunE: withAuth("albums.delete", func(ctx *CmdContext) error {
		albumID := ctx.Args[0]
		return ctx.runMutation(mutationSpec{
			Command:  "albums.delete",
			Method:   "flickr.photosets.delete",
			Resource: map[string]any{"album_id": albumID},
			PlanMsg:  fmt.Sprintf("Would delete album %s\n", albumID),
			PlanData: map[string]any{"id": albumID},
		}, func() (any, error) {
			if err := ctx.Client.Call(ctx.Cmd.Context(), "flickr.photosets.delete", map[string]string{"photoset_id": albumID}, nil); err != nil {
				return nil, ctx.R.Failure(ctx.Meta, output.Errorf(model.ErrFlickrAPI, "%v", err))
			}
			ctx.R.Human("Deleted album %s\n", albumID)
			return map[string]any{"id": albumID}, nil
		})
	}),
}
...
var albumsAddPhotosCmd = &cobra.Command{
	...
	RunE: withAuth("albums.add-photos", func(ctx *CmdContext) error {
		...
		return ctx.runMutation(mutationSpec{
			...
			Resource: map[string]any{"album_id": albumID, "photo_ids": photoIDs},
			PlanMsg:  fmt.Sprintf("Would add %d photos to album %s\n", len(photoIDs), albumID),
			PlanData: map[string]any{"album_id": albumID, "photo_ids": photoIDs},
		}, func() (any, error) {
			var added []string
			...
			if len(errs) > 0 {
				...
				return nil, ctx.R.Failure(ctx.Meta, output.ErrorWithDetails(
					model.ErrFlickrAPI,
			...
			return map[string]any{"album_id": albumID, "added": added}, nil
		})
	}),
}
...
var albumsRemovePhotosCmd = &cobra.Command{
	...
	RunE: withAuth("albums.remove-photos", func(ctx *CmdContext) error {
		...
		return ctx.runMutation(mutationSpec{
			...
			Resource: map[string]any{"album_id": albumID, "photo_ids": photoIDs},
			PlanMsg:  fmt.Sprintf("Would remove %d photos from album %s\n", len(photoIDs), albumID),
			PlanData: map[string]any{"album_id": albumID, "photo_ids": photoIDs},
		}, func() (any, error) {
			var removed []string
			var errs []map[string]any
			for _, photoID := range photoIDs {
				params := map[string]string{"photoset_id": albumID, "photo_id": photoID}
			...
			return map[string]any{"album_id": albumID, "removed": removed}, nil
		})
	}),
}

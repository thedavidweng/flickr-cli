...
func TestScanStableOrder(t *testing.T) {
	...
	for _, f := range files {
		_ = os.WriteFile(filepath.Join(dir, f), []byte("test"), 0o644)
	}
...
func TestScanInvalidExtensions(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "photo.jpg"), []byte("test"), 0o644)
	_ = os.WriteFile(filepath.Join(dir, "doc.txt"), []byte("test"), 0o644)
	_ = os.WriteFile(filepath.Join(dir, "image.png"), []byte("test"), 0o644)
...
func TestScanRecursive(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "a.jpg"), []byte("test"), 0o644)
	_ = os.WriteFile(filepath.Join(subdir, "b.jpg"), []byte("test"), 0o644)
...
func TestScanCustomExtensions(t *testing.T) {
	dir := t.TempDir()
	_ = os.WriteFile(filepath.Join(dir, "photo.jpg"), []byte("test"), 0o644)
	_ = os.WriteFile(filepath.Join(dir, "photo.raw"), []byte("test"), 0o644)
